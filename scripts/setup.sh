...
if [[ "$mode_simple" == "true" || "$mode_lab" == "true" ]]; then
  ...
  zone_letter="$(ask_input 'GCP Zone Letter (a/b/c/f/etc.)' 'b')"
fi
...
print_success "Wrote $TFVARS (mode 600 - contains the admin password)"
echo
...
if os.path.exists(yaml_path):
    ...
    if license_key:
        ...
        # Check if the existing license key in the file is non-empty.
        # We look for a line starting with license_key followed by non-empty quotes.
        sq = chr(39)
    ...
    with open(yaml_path, "w", encoding="utf-8") as f:
        f.write(content)
'
print_success "Updated pexip-config.yaml with the Pexip license key and domain configuration"
echo
...
print_info "This takes ~8-12 minutes total:"
print_info "  1) Copy Pexip images into your project (~2-3 min each, runs in parallel)"
print_info "  2) Create VPC, firewall, addresses     (~30 sec)"
print_info "  3) Boot Management Node + wait for API (~3-5 min)"
print_info "  4) Register and boot Conferencing Nodes (~2-3 min)"
print_info "Terraform prints '...still creating...' every 10s. That's normal."
echo

# Cloud Shell occasionally returns 'connection refused' to
# compute.googleapis.com mid-apply (transient blip). Re-running picks up
# where we left off: terraform's state file remembers what was created.
# We retry up to 3 times, then fall back to -refresh=false which skips
# the per-resource GET step that's most often the failure point.
...
for attempt in $(seq 1 $apply_attempts); do
  echo
  echo "==> terraform apply (attempt $attempt of $apply_attempts)..."
  if terraform apply -auto-approve -parallelism=4; then
    apply_ok=1
    break
  fi
...
if [[ "${apply_ok:-0}" != "1" ]]; then
  ...
  if ! terraform apply -auto-approve -parallelism=4 -refresh=false; then
    print_error_banner "✖  Terraform Deployment Failed" \
                       "All retry attempts were exhausted." \
                       "$TEXT_RED"
    cat <<EOF
If the errors mention 'alreadyExists' / HTTP 409, your project has
leftover resources from a previous run.

If the errors mention 'connection refused', it's a Cloud Shell network
blip. Wait a minute and try again from the terraform directory:
  cd $TF_DIR && terraform apply -auto-approve -parallelism=4

EOF
    echo
    if ask_confirm "Would you like to run the nuke script to wipe the leftover resources?" "n"; then
      echo
      print_info "Running cleanup nuke script..."
      "${REPO_ROOT}/scripts/nuke.sh" "$project_id"
    else
      print_info "Leaving resources intact. To manually wipe later, run: ${REPO_ROOT}/scripts/nuke.sh"
    fi
    exit 1
  else
    apply_ok=1
