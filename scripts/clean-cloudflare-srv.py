...
def main():
    ...
    zone_name = os.environ.get("ACME_ZONE_NAME") or config.get("cloudflare_zone_name") or domain

    if not token:
        print("Error: Cloudflare API token is empty or not provided via CF_DNS_API_TOKEN / terraform.tfvars")
        sys.exit(1)
    if not domain:
        print("Error: ACME domain is empty or not provided via ACME_DOMAIN / terraform.tfvars")
        sys.exit(1)

    print(f"Cloudflare API Token found. Target domain: {domain}, Zone name: {zone_name}")
    ...
    records_url = f"https://api.cloudflare.com/client/v4/zones/{zone_id}/dns_records?per_page=100"
    ...
    # Parse hostnames
    manager_host = os.environ.get("ACME_MANAGER_HOSTNAME") or config.get("acme_manager_hostname") or "pexip-mgr"
    conf_prefix = os.environ.get("ACME_CONF_HOSTNAME_PREFIX") or config.get("acme_conf_hostname_prefix") or "pexip-conf"
    ...
    patterns = [
        ...
        re.compile(rf"^_acme-challenge\.{conf_prefix_esc}-\d+\.{domain_esc}$", re.IGNORECASE),
    ...
    if not to_delete:
        if is_check:
            ...
            print("No conflicting or orphaned Pexip DNS records found in Cloudflare. You are good to go!")
            return
