...
def main():
    ...
    group.add_argument("--check", action="store_true", help="Check if licenses are active (exit code 10 if active)")
    group.add_argument("--deactivate", action="store_true", help="Deactivate and return all active licenses")
    ...
    if not host:
        print_info("No Pexip Management Node IP address found in Terraform state. Nothing to check.")
        sys.exit(0)

    if not password:
        print_error("Pexip admin password could not be discovered. Please specify --password.")
        sys.exit(1)
    ...
    print_info(f"Checking if Management Node at {check_host}:{check_port} is reachable...")
    if not is_port_open(check_host, check_port, timeout=2):
        print_info(f"Pexip Management Node port {check_port} is unreachable (offline or already deleted). Skipping license check.")
        sys.exit(0)
    ...
    try:
        ...
        if args.deactivate:
            ...
            for entitlement, lic in unique_entitlements.items():
                ...
                print_info(f"Returning license entitlement {entitlement} via DELETE {resource_uri}...")
                
                del_res = session.delete(delete_url, verify=args.verify_ssl, timeout=15)
                if del_res.status_code in (200, 204):
                    print_success(f"Successfully returned license entitlement {entitlement}.")
                else:
                    print_error(f"Failed to return license entitlement {entitlement}: HTTP {del_res.status_code}")
                    try:
        ...
        print_error(f"Failed to connect to Pexip Management Node at https://{host}. Error: {e}")
        sys.exit(1)
    except Exception as e:
